...
from app.api.routes.chatmap.chatmap import get_user_chatmaps, CHATMAP_API_BASE_URL
...
class TestGetUserChatMaps:
    ...
    @pytest.mark.asyncio
    async def test_get_user_chatmaps_success(self):
        ...
        with patch("httpx.AsyncClient") as mock_client:
            ...
            result = await get_user_chatmaps(request=mock_request, user=mock_user)

            assert result == {"maps": MOCK_MAPS_RESPONSE}
            assert len(result["maps"]) == 2

            call_args = mock_client.return_value.__aenter__.return_value.get.call_args
            assert f"{BASE_URL}/user/test-hanko-uuid/map" in call_args[0][0]

            cookies = call_args[1]["cookies"]
            assert cookies["hanko"] == "test-hanko-jwt"

    @pytest.mark.asyncio
    async def test_get_user_chatmaps_empty(self):
    ...
    @pytest.mark.asyncio
    async def test_get_user_chatmaps_no_cookie(self):
        ...
        with pytest.raises(Exception) as exc_info:
            await get_user_chatmaps(request=mock_request, user=mock_user)

        assert exc_info.value.status_code == 401
        assert "cookie" in exc_info.value.detail.lower()

    @pytest.mark.asyncio
    async def test_get_user_chatmaps_upstream_error(self):
    ...
    @pytest.mark.asyncio
    async def test_get_user_chatmaps_connection_error(self):
