...
export class HankoAuth extends LitElement {
  ...
  static styles = css`
    ...
    /* Dropdown positioning - align right edge to right edge of trigger */
  ...
  render() {
    ...
    if (this.loading) {
      // skeleton button to avoid moving elements
      return html`
        <wa-button appearance="plain" disabled> Log in </wa-button>
      `;
    }

    if (this.error) {
      return html` <wa-button disabled>Login error</wa-button> `;
    }
    ...
    if (this.user) {
      ...
      if (this.showProfile) {
        ...
        return html`
          <wa-button href="${loginUrl}" appearence="plain">Log In</wa-button>
        `;
      }
    }
  }
