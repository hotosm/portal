...
function mapToProject(item: FieldTMApiProject): FieldTMProject {
  return {
    id: item.id,
    title: item.title,
    href: `https://fmtm.hotosm.org/projects/${item.id}`,
...
export function useFieldTMProjects() {
  return useQuery({
    ...
    queryFn: async (): Promise<FieldTMProject[]> => {
      const response = await fetch("/api/fmtm/user/projects", {
        credentials: "include",
      });

      if (!response.ok) {
        if (response.status === 401 || response.status === 403) return [];
        throw new Error(`[${response.status}] Failed to fetch Field TM projects`);
      }
