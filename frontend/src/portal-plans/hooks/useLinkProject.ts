...
export function useLinkProject({ open, app }: { open: boolean; app: AppName | null }) {
  ...
  function handleProjectAdded(project: ProjectOption, _key: string) {
    // A task with no tool yet accepts a URL from any of them.
    if (app !== null && project.app !== app) {
  ...
  function handleAddUrl() {
    handleAddUrlBase({
      localSelected: new Set(selected ? [`${selected.app}:${selected.project_id}`] : []),
