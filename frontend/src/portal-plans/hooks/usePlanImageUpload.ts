...
export function usePlanImageUpload({ planId, initialImages = [] }: UsePlanImageUploadOptions) {
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (planId) {
      setIsUploading(true)
      try {
        const uploaded = await Promise.all(optimized.map((f) => uploadMutation.mutateAsync(f)))
  ...
  function handleRemoveImage(id: string) {
    if (planId) {
      ...
      const idx = Number.parseInt(id.replace('pending-', ''), 10)
